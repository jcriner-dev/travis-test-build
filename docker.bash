...
# travis puts your python in a virtualenv by default
    ...
    echo "begin $UNAME install ..."
    if [[ -n "$DOCKER_32_BIT" ]]; then
        ### ADD ###
        # do everything required to get the docker image up to snuff
        ...
        # pipsi install pipenv; or sudo; or whatever
        # $PIP install pipenv --upgrade
        ### END ADD ###
    else
        if [[ -n "$VIRTUAL_ENV" ]]; then
            echo "ERROR: installing pipenv inside of a virtualenv not supported."
            exit 1
        fi
        $PIP install pipenv --upgrade
    fi
